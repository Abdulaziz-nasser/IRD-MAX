...
The first confirmed intersection color stays locked for this run:
  BLUE -> LEFT at every corner; ORANGE -> RIGHT at every corner.
After locking, only that color can request another corner turn.
Restart this Python program to clear the color lock for a new run.
...
TUNE = {
    ...
    "DRIVE_SPEED": int(os.environ.get("DRIVE_SPEED", "45")),
    "TURN_SPEED": int(os.environ.get("TURN_SPEED", "38")),
    "POST_TURN_SPEED": int(os.environ.get("POST_TURN_SPEED", "40")),
    "POST_TURN_SEC": float(os.environ.get("POST_TURN_SEC", ".5")),
    "SEE_COLOR_SPEED": int(os.environ.get("SEE_COLOR_SPEED", "40")),
    "MIN_SPEED": int(os.environ.get("MIN_SPEED", "55")),
    ...
    "WALL_AVOID_MIN_BIAS_DEG": float(os.environ.get("WALL_AVOID_MIN_BIAS_DEG", "1.0")),
    ...
    "DETECT_COOLDOWN_S": float(os.environ.get("DETECT_COOLDOWN_S", "1.5")),
    ...
    "COLOR_ROI_WIDTH_FRAC": float(os.environ.get("COLOR_ROI_WIDTH_FRAC", "0.35")),
    ...
    # --- [STRAIGHT] yaw centering controller ---
    ...
    # Wall-distance terms below are unchanged; yaw is handled by [STRAIGHT].
    ...
    "FINAL_ALIGN_SPEED": int(os.environ.get("FINAL_ALIGN_SPEED", "40")),
    ...
    "FINAL_ALIGN_ROI_H_FRAC": float(os.environ.get("FINAL_ALIGN_ROI_H_FRAC", "0.42")),
    "FINAL_ALIGN_DETECT_DELAY_S": float(os.environ.get("FINAL_ALIGN_DETECT_DELAY_S", ".50")),
}
...
def mix_straight_steering(yaw_cmd, d_left_cm, d_right_cm,
                          active_side, tune):
    """Keep normal straight steering yaw-only, with a latched close-wall guard.

    Returns ``(offset_deg, active_side, guard_deg)``.  Positive logical
    offsets steer away from a left wall; negative offsets steer away from a
    right wall.  The guard cannot switch sides until the active wall reaches
    the configured OFF distance, preventing rapid left/right sensor chasing.
    """
    max_offset = max(0.0, float(tune["STEER_MAX_DEG"]))
    yaw_cmd = max(-max_offset, min(max_offset, float(yaw_cmd)))
    on_cm = max(0.0, float(tune["WALL_AVOID_ON_CM"]))
    off_cm = max(on_cm, float(tune["WALL_AVOID_OFF_CM"]))
    ...
    left = float(d_left_cm) if valid_cm(d_left_cm) else -1.0
    right = float(d_right_cm) if valid_cm(d_right_cm) else -1.0
    side = active_side if active_side in ("L", "R") else None

    if side is not None:
        active_distance = left if side == "L" else right
        if active_distance < 0.0 or active_distance >= off_cm:
            side = None

    if side is None:
        close_sides = []
        if left >= 0.0 and left <= on_cm:
            close_sides.append((left, "L"))
        if right >= 0.0 and right <= on_cm:
            close_sides.append((right, "R"))
        if close_sides:
            side = min(close_sides, key=lambda item: item[0])[1]

    guard_cmd = 0.0
    if side is not None and on_cm > 0.0:
        active_distance = left if side == "L" else right
        if active_distance >= 0.0:
            proximity = max(0.0, min(1.0,
                            (on_cm - active_distance) / on_cm))
            minimum = max(0.0, min(max_offset,
                          float(tune["WALL_AVOID_MIN_BIAS_DEG"])))
            maximum = max(minimum,
                          float(tune["WALL_AVOID_MAX_BIAS_DEG"]) *
                          float(tune["WALL_AVOID_GAIN"]))
            requested = minimum + proximity * (maximum - minimum)
            magnitude = min(max_offset, max(0.0, requested))
            guard_cmd = magnitude if side == "L" else -magnitude

    # A real close-wall event may override an opposing yaw command, but normal
    # valid side-distance changes never alter yaw-only straight steering.
    if guard_cmd > 0.0:
        offset = max(yaw_cmd, guard_cmd)
    elif guard_cmd < 0.0:
        offset = min(yaw_cmd, guard_cmd)
    else:
        offset = yaw_cmd
    offset = max(-max_offset, min(max_offset, offset))
    return offset, side, guard_cmd


class StraightWallGuard:
    """Stateful close-wall guard that can be reset between straight sections."""
    ...
    def update(self, yaw_cmd, d_left_cm, d_right_cm):
        offset, self.active_side, guard_cmd = mix_straight_steering(
            yaw_cmd, d_left_cm, d_right_cm, self.active_side, self.tune)
        return offset, self.active_side, guard_cmd
...
def main():
    ...
    straight_wall_guard = StraightWallGuard(TUNE)
    WA_IDLE, WA_ACTIVE = 0, 1
    wall_avoid_state = WA_IDLE
    wall_avoid_side = None
    last_straight_log_t = -1e9
    ...
    def reset_straight_controls():
        nonlocal wall_avoid_state, wall_avoid_side, last_straight_log_t
        heading_hold.reset()
        straight_wall_guard.reset()
        wall_avoid_state = WA_IDLE
        wall_avoid_side = None
        last_straight_log_t = -1e9
    ...
    def read_tlm():
        ...
        try:
            ...
            while b"\n" in rx_buf:
                ...
                if m:
                    ever_had_tlm = True
                    last_yaw = wrap180(float(m.group(1)) % 360.0)
    ...
    # Run-wide lock: do not clear this when an individual turn finishes.
    first_intersection_color = None
    ...
    # [STRAIGHT] controller state lives in heading_hold / straight_wall_guard.
    last_tx = 0.0
    last_spd_sent = 0

    repel_left_on = False
    repel_right_on = False
    ...
    def continuous_heading_active():
        return state in (STATE_CENTER, STATE_FINAL_BURST,
                         STATE_FINAL_FWD, STATE_FINAL_ALIGN)

    def send_center(steer_deg, speed_pwm):
        nonlocal last_spd_sent
        if continuous_heading_active() and speed_pwm > 0:
            steer_deg = heading_hold.limit_command(
                steer_deg, time.monotonic(), +1)
    ...
    while True:
        ...
        if not pause_active:
            ...
            if detect_enabled and (not recent_turn) and (not in_blackout) and state in (STATE_CENTER, STATE_FINAL_FWD):
                # Once locked, ignore the opposite color even if it has more pixels.
                # Still require a fresh detection of the locked color at each corner.
                if first_intersection_color == "BLUE":
                    cand_color = "BLUE" if pass_bl else None
                elif first_intersection_color == "ORANGE":
                    cand_color = "ORANGE" if pass_or else None
                elif pass_bl or pass_or:
        ...
        # ---- STRICT COLOR LATCH → reuse the first intersection's color ----
        if (not pause_active) and detect_enabled \
           and len(color_q) == color_q.maxlen and len(color_q)>0 and all(c == color_q[0] for c in color_q):
            ...
            selected_color = first_intersection_color or seen_color
            raw_dir = TUNE["COLOR_TURN_MAP"].get(selected_color, None)
            if raw_dir in ("LEFT","RIGHT"):
                if first_intersection_color is None:
                    first_intersection_color = selected_color
                    print(f"[COLOR LOCK] first={first_intersection_color} -> {raw_dir} for all corners")
                ...
                turn_requested = True
                print(f"[LATCH] color={first_intersection_color} dir={turn_latched_dir} -> target={last_turn_target:.1f}°")
        ...
        # ===== TURN COMPLETE → clear only the per-corner latch; keep the first color =====
        if state == STATE_WAIT_TURN:
            ...
            if reached_angle or timed_out:
                ...
                last_turn_target = None
                turn_latched_dir = None  # first_intersection_color stays locked
                turn_requested = False
                turn_count += 1
        ...
        # ---------------- Lane controller (centering) ----------------
        steer_deg = TUNE["CENTER_DEG"]
        if state in (STATE_CENTER, STATE_FINAL_BURST, STATE_FINAL_FWD, STATE_FINAL_ALIGN):
            yaw_target = heading_hold.update(
                last_yaw, yaw_ref, last_yaw_t, state,
                float(TUNE["STEER_KP_DEG_PER_DEG"]),
                float(TUNE["STEER_MAX_DEG"]))
            yaw_cmd = yaw_target - float(TUNE["CENTER_DEG"])
            ...
            # [STRAIGHT] uses yaw alone during normal driving.  Side sensors
            # only intervene inside the close-wall emergency threshold.
            previous_wall_side = wall_avoid_side
            offset_total, wall_avoid_side, wall_guard_cmd = straight_wall_guard.update(
                yaw_cmd, dL, dR)
            wall_avoid_state = (WA_ACTIVE if wall_avoid_side is not None
                                else WA_IDLE)
            if wall_avoid_side != previous_wall_side:
                if wall_avoid_side is None:
                    print("[WALL_AVOID] STOP")
                else:
                    near = dL if wall_avoid_side == 'L' else dR
                    print(f"[WALL_AVOID] START side={wall_avoid_side} at {near:.1f}cm")

            # The normal 18 cm repulsion and dR-dL lane centering caused the
            # alternating corrections seen in the test video, so neither is
            # mixed into [STRAIGHT].
            repel_left_on = False
            repel_right_on = False
            target = TUNE["CENTER_DEG"] + SERVO_DIR * offset_total
            clamp_min = TUNE["CENTER_STEER_MIN"]; clamp_max = TUNE["CENTER_STEER_MAX"]
            wall_active_small = (wall_avoid_state == WA_ACTIVE or
                                 side_slow_active or contact_active)
            if wall_active_small and TUNE["WALL_STEER_EXPAND_CENTER_CLAMP"]:
                clamp_min = int(TUNE["WALL_STEER_CENTER_MIN"]); clamp_max = int(TUNE["WALL_STEER_CENTER_MAX"])
            steer_deg = float(max(clamp_min, min(clamp_max, target)))
            steer_deg = float(max(LEFT_LIMIT, min(RIGHT_LIMIT, steer_deg)))

            if state == STATE_CENTER and (now - last_straight_log_t) >= 0.50:
                print("[STRAIGHT] yaw=%.1f ref=%.1f err=%+.1f "
                      "yaw_cmd=%+.2f wall=%s guard=%+.2f target=%.2f" %
                      (last_yaw, yaw_ref, shortest_err(yaw_ref, last_yaw),
                       yaw_cmd, wall_avoid_side or "-", wall_guard_cmd,
                       steer_deg))
                last_straight_log_t = now
        ...
        locked_dir = TUNE["COLOR_TURN_MAP"].get(first_intersection_color, "-")
        lock_text = (f"LOCK: {first_intersection_color} -> {locked_dir}"
                     if first_intersection_color else "LOCK: waiting for first intersection")
        cv.putText(vis, lock_text, (10,90),
        ...
        if key in (ord('q'), ord('Q')):
            ...
            if not yaw_ready():
                safe_write(b"STOP\n")
                print("[YAW] Start blocked: waiting for fresh Arduino yaw telemetry.")
                continue
            if yaw_start is None: yaw_start = float(last_yaw)
            yaw_ref = float(yaw_start)
            safe_write(b"START\n")
